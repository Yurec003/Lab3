...
public class AStarState
{
    ...
    private HashMap<Location, Waypoint> close_waypoints = new HashMap<>();
    ...
    public Waypoint getMinOpenWaypoint() {
        if (open_waypoints.size() == 0) {
            return null;
        }
        Waypoint minWaypoint = null;
        boolean isFirstWaypoint = true;
        for (Map.Entry<Location, Waypoint> point : open_waypoints.entrySet()){
            Waypoint waypoint = point.getValue();
            if (isFirstWaypoint){
                minWaypoint = waypoint;
                isFirstWaypoint = false;
            }
            if (waypoint.getPreviousCost() < minWaypoint.getPreviousCost()){
                minWaypoint = waypoint;
            }
        }
        return minWaypoint;
    ...
    public void closeWaypoint(Location loc) {
        ...
        close_waypoints.put(loc, open_waypoints.get(loc));
    }
